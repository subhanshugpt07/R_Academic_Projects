...
library(rjson)
library(jsonlite)
auto<- ndjson::stream_in("musical.json")
View(auto)
library(quanteda)
library(stm)
library(tm)
...
#Generate DFM
autoold <- corpus(auto$reviewText,
                     docvar=data.frame(name=auto$reviewerName))



corpus<- toLower(autoold, keepAcronyms = FALSE) 
...
dfm.tm
require(tm)
dtmss <- removeSparseTerms(dfm.tm, 0.81)
dtmss
...
autoold <- gsub("'", "", autoold) # remove apostrophes
autoold <- gsub("[[:punct:]]", " ", autoold)  # replace punctuation with space
autoold <- gsub("[[:cntrl:]]", " ", autoold)  # replace control characters with space
autoold <- gsub("^[[:space:]]+", "", autoold) # remove whitespace at beginning of documents
autoold <- gsub("[[:space:]]+$", "", autoold) # remove whitespace at end of documents
autoold <- gsub("[^a-zA-Z -]", " ", autoold) # allows only letters
autoold <- tolower(autoold)  # force to lowercase

## get rid of blank docs
autoold <- autoold[autoold != ""]

# tokenize on space and output as a list:
doc.list <- strsplit(autoold, "[[:space:]]+")
...
serVis(json, out.dir = 'amazon auto', open.browser = TRUE)
****************************************************************************************************************
